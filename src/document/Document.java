...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		boolean foundPreviousVowel = false;
		int syllableCount = 0;
		for (int i = 0; i < word.length(); i++) {
			char c = word.charAt(i);
			if (i != word.length() - 1) {
				if (isVowel(c)) {
					foundPreviousVowel = true;
				}
				else if (!isVowel(c) && foundPreviousVowel) {
					syllableCount++;
					foundPreviousVowel = false;
				}
			}
			else {
				if (!foundPreviousVowel && (c == 'e' || c == 'E')) {
					if (syllableCount == 0) {
						syllableCount = 1;
					}
				}
				else if (foundPreviousVowel || (isVowel(c) && (c != 'e' && c != 'E'))) {
					syllableCount++;
					foundPreviousVowel = false;
				}
			}
	...
	public double getFleschScore()
	{
		...
		double score = 206.835 - 1.015*((double)getNumWords()/getNumSentences()) 
